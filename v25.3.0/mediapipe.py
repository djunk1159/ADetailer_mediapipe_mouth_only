...
def mediapipe_predict(
    ...
    mapping = {
        ...
        "mediapipe_face_mesh_mouth_only": mediapipe_face_mesh_mouth_only,
    }
...
def mediapipe_face_mesh_mouth_only(
    image: Image.Image, confidence: float = 0.3
) -> PredictOutput[int]:
    import mediapipe as mp
    ...
    left_idx = np.array(list(mp_face_mesh.FACEMESH_LIPS)).flatten()
    right_idx = np.array(list(mp_face_mesh.FACEMESH_LIPS)).flatten()
    ...
    with mp_face_mesh.FaceMesh(
        ...
        for landmarks in pred.multi_face_landmarks:
            ...
            left_m = points[left_idx]
            right_m = points[right_idx]
            left_outline = cv2.convexHull(left_m).reshape(-1).tolist()
            right_outline = cv2.convexHull(right_m).reshape(-1).tolist()
